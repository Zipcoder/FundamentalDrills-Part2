...
public class StringsAndThings {
    ...
    public Integer countYZ(String input) {
        ...
        for (String name : splitWords) {
            if (name.charAt(name.length() - 1) == 'y' || name.charAt(name.length() - 1) == 'z') {
                count++;
            }
        }
        return count;
    ...
    public Boolean equalIsNot(String input) {
        int notCounter = input.split("is", -1).length - 1;
        int isCounter = input.split("not", -1).length - 1;
